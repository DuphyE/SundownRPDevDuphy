...
elseif GetResourceState('rsg-core') == 'started' then
    ...
    Citizen.CreateThread(function()
        while true do
            ...
            print("^1[ERROR]^0 Make sure to start ^3sdrp_idcard^0 after the frameworks in your ^2server.cfg^0 file.")
        end
    end)
end
    ...
    elseif Framework == "RSG" then
        ...
        function FXRemoveMoney(src,moneytype,count)
            ...
            if moneytype == "cash" then
                Player.Functions.RemoveMoney('cash', count, 'sdrp_idcard')
            else
                Player.Functions.RemoveMoney('bank', count, 'sdrp_idcard')
            end
            return true
        end
        function FXAddMoney(src,moneytype,count)
            local Player = RSGCore.Functions.GetPlayer(src)
            if moneytype == "cash" then
                Player.Functions.AddMoney('cash', count, 'sdrp_idcard')
            else
                Player.Functions.AddMoney('bank', count, 'sdrp_idcard')
            end
            return true
        end
