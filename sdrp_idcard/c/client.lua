...
RegisterNUICallback('print',function(data)
    ...
    if data.imgLink then
        TriggerServerEvent('sdrp_idcard:server:print', data.imgLink)
    end
end)

RegisterNetEvent('sdrp_idcard:client:setData',function(data)
    idcardData = data
end)

RegisterNUICallback('createIdCard',function(data)
   TriggerServerEvent('sdrp_idcard:server:buyIdCard',data)
end)
...
RegisterNetEvent('sdrp_idcard:client:updateData',function()
    TriggerServerEvent('sdrp_idcard:server:GetData')
end)

RegisterNetEvent('sdrp_idcard:client:showIDCardSQL',function()
    if idcardData then
        TriggerServerEvent('sdrp_idcard:server:ShowIdCard',idcardData)
    else
        TriggerServerEvent('sdrp_idcard:server:GetData')
        Wait(2000)
        TriggerEvent('sdrp_idcard:client:showIDCardSQL')
    end
end)

RegisterNetEvent("sdrp_idcard:client:PreviewPhoto", function(typee, data)
...
RegisterNetEvent("sdrp_idcard:client:CreateIdcardUi", function(data, illegal)
    creating = true
    Config.HideHud()
...
RegisterNetEvent("sdrp_idcard:client:ShowUi", function(typee, data)
    local closestPlayers = GetClosestPlayer()
    if typee == "photo" then
        if #closestPlayers > 0 then
            TriggerServerEvent("sdrp_idcard:server:ShowUi", closestPlayers, typee, data)
        else
            SetNuiFocus(true,true)
            SendNUIMessage({
                action = 'showphoto',
...
local function takePhoto(v)
    ...
    Wait(1000)
    TriggerServerEvent('sdrp_idcard:server:setBucket',GetPlayerServerId(PlayerId()))
    ...
    Citizen.CreateThread(function()
        while cam do
            ...
            if IsDisabledControlPressed(0,Config.Keybinds["exit"]) then
                ...
                TriggerServerEvent('sdrp_idcard:server:setBucket',0)
                Config.ShowHud()
        ...
        for k,v in pairs(Config.IDCardNPC) do
            ...
            if dist < v.distance and v.canInteract then
                ...
                if PromptHasHoldModeCompleted(movements2[1]) then
                    ...
                    TriggerServerEvent('sdrp_idcard:server:useImagePlease', k)
                end
            end
        end
        Wait(sleep)
